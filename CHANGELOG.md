...
### Fixed
- **[Critical] `z.date().min()` with a future date threw `GENERATION_FAILED`** — The default upper bound for date generation was the hardcoded `ANCHOR_MS` (2026-01-01). Any `.min()` set to a date after that anchor made `minMs > maxMs`, triggering an immediate error. The default upper bound is now `Date.now()`.
- **[Major] Narrow exclusive float ranges could produce invalid values** — `generateNumber` rounds to `toFixed(4)`. For ranges narrower than `~5e-4`, rounding could snap a value back to exactly the exclusive `gt`/`lt` bound, causing `safeParse` to fail. A post-rounding clamp now ensures the value always stays strictly inside exclusive bounds.
- **[Major] `z.literal("a", "b", "c")` in Zod v4 always generated the first value** — `getLiteralValue` returned `def.values[0]` unconditionally. The dispatch now uses `ctx.rng.pick(getLiteralValues(schema))` so all literal variants are reachable. Added `getLiteralValues()` helper to `compat.ts`.
- **[Minor] `MatcherContext.leaf` contained the schema description instead of the field name** — When `.describe("email")` was used on a field named `x`, `ctx.leaf` inside a matcher's `generate()` was `"email"` instead of `"x"`. The pattern match still uses the semantic hint (description ?? field name) for triggering, but `ctx.leaf` is now always the actual path leaf key.
- **[Minor] `withConfig({ matchers })` silently dropped plugin matchers** — Calling `withConfig` with a `matchers` array but no `plugins` caused `pluginMatchers` to be rebuilt as `[]` inside `configure()`, removing all previously installed plugin matchers for the duration of `fn()`. `withConfig` now merges the new matchers with the current global matchers when no `plugins` key is provided.
