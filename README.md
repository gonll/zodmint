...
When you pass `matchers` without `plugins`, any plugin matchers already installed via `configure({ plugins })` are preserved and take lower priority than the new matchers. Pass `plugins: []` explicitly if you want to clear plugins for the duration of the callback.
...
The `generate` function receives an optional `MatcherContext` with the full schema path and the matched leaf key. Use it to produce different values depending on where in the schema the field appears. `ctx.leaf` is always the actual field name from the path — if the field uses `.describe()`, the description is used for pattern matching but `ctx.leaf` stays as the field name.
...
`z.literal(val)` returns the literal value. In Zod v4, multi-value literals (`z.literal("a", "b", "c")`) pick randomly from all provided values. `z.enum([...])` and `z.nativeEnum(E)` pick a random value.
