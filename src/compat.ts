...
/**
 * Returns all possible literal values as an array.
 * v3: single-element array [def.value]
 * v4: def.values (Array or Set, may contain multiple values for z.literal("a","b","c"))
 */
export function getLiteralValues(schema: z.ZodTypeAny): unknown[] {
  const def = rawDef(schema);
  // v4: values is an Array (multi-value literal: z.literal("a", "b", "c"))
  if (Array.isArray(def.values)) {
    return def.values as unknown[];
  }
  // v4 (older builds): values is a Set
  if (def.values instanceof Set) {
    return [...(def.values as Set<unknown>)];
  }
  // v3: single value
  return [def.value];
}
...
/**
 * Returns a single literal value (first element). Kept for backward compatibility.
 * Prefer getLiteralValues() + rng.pick() when randomness is available.
 */
export function getLiteralValue(schema: z.ZodTypeAny): unknown {
  return getLiteralValues(schema)[0];
}
