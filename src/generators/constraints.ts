...
export function generateNumber(
  ...
  // Generate a float rounded to 4 decimal places, then clamp back into the
  // exclusive bounds. toFixed(4) can round a value that is very close to an
  // exclusive bound back to exactly that bound, causing safeParse to fail.
  // Clamping ensures we always stay strictly inside (gt/lt) bounds.
  const raw = parseFloat(rng.nextFloat(min, max).toFixed(4));
  const exclusiveMin = (c.gt !== undefined || c.positive) && !c.int;
  const exclusiveMax = c.lt !== undefined && !c.int;
  if (exclusiveMin && raw <= min) return min + 0.0001;
  if (exclusiveMax && raw >= max) return max - 0.0001;
  return raw;
}
...
export function generateDate(c: DateConstraints, rng: SeededRNG, path: string[]): Date {
  // Default max to now (not the hardcoded ANCHOR_MS) so that schemas with
  // .min(futureDate) do not produce minMs > maxMs and throw GENERATION_FAILED.
  const defaultMaxMs = Date.now();
  const maxMs = c.max?.getTime() ?? defaultMaxMs;
